...
us_mapping <-
  merge(readOGR("tl_2019_us_state", stringsAsFactors = F),
        us_mapping,
        by = "NAME")
us_map_data <- us_mapping@data
rownames(us_map_data) <- sapply(us_mapping@polygons, function(x) x@ID)
us_map_simple <- gSimplify(us_mapping, tol = 0.01, topologyPreserve = TRUE)
us_map_new <- SpatialPolygonsDataFrame(us_map_simple, us_map_data)
...
pal <- colorNumeric(palette = "YlOrRd",
                    domain = c(
                      min(us_map_new$LOGCASES, na.rm = T),
                      max(us_map_new$LOGCASES, na.rm = T)
                    ),
                    na.color = "white")
lab <- paste0(
  "<strong>",
  us_map_new$NAME,
  "</strong><br>",
  prettyNum(us_map_new$CASES, big.mark = ","),
  " cases"
) %>%
