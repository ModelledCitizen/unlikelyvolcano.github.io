...
jhu_url <- paste0(jhu_url, "csse_covid_19_daily_reports/%s.csv")
tryCatch({
  update_date <- format.Date(Sys.Date(), "%d %B %Y")
  daily_url <- sprintf(jhu_url, format.Date(Sys.Date(), "%m-%d-%Y"))
  daily_update <- read.csv(daily_url, stringsAsFactors = F)
},
error = function(cond) {
  update_date <<- format.Date(Sys.Date() - 1, "%d %B %Y")
  daily_url <<- sprintf(jhu_url, format.Date(Sys.Date() - 1, "%m-%d-%Y"))
  daily_update <<- read.csv(daily_url, stringsAsFactors = F)
})
```
...
error = function(cond) {
  ...
  write.csv(cop[, c("UpdatedDate", "Result.Group", "Result", "Result.Date")], "phila_data.csv")
})
